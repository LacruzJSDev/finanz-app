...
class ApiFalsa {
  ...
  getTransactionsApiV1AccountsAccountIdTransactionsGet() {
...
describe('TransactionsService, respuestas fuera de orden', () => {
  ...
  it('ignora la lista de la cuenta anterior aunque llegue la última', () => {
    service.getTransactions('cuenta-A', 20, 0).subscribe();
    service.getTransactions('cuenta-B', 20, 0).subscribe();
  ...
  it('no pega una página al final de una lista que ya se recargó', () => {
    ...
    service.getTransactions('cuenta-A', 20, 2).subscribe(); // el scroll pide más
    service.getTransactions('cuenta-A', 20, 0).subscribe(); // y se guarda un movimiento
  ...
  it('sí acumula la página siguiente cuando nadie ha recargado en medio', () => {
    ...
    service.getTransactions('cuenta-A', 20, 2).subscribe();
  ...
  it('no deja loadingMore encendido cuando su página se descarta', () => {
    ...
    service.getTransactions('cuenta-A', 20, 1).subscribe(); // scroll en vuelo
    expect(service.loadingMore()).toBe(true);

    service.getTransactions('cuenta-A', 20, 0).subscribe(); // recarga que lo invalida
