...
constructor({ accessToken, apiKeys, basePath, credentials, encodeParam, encoder, password, username, withCredentials }: ConfigurationParameters = {}) {
        ...
        // init default RefreshTokenCookie credential
        if (!this.credentials['RefreshTokenCookie']) {
            this.credentials['RefreshTokenCookie'] = () => {
                if (this.apiKeys === null || this.apiKeys === undefined) {
                    return undefined;
                } else {
                    return this.apiKeys['RefreshTokenCookie'] || this.apiKeys['refresh_token'];
                }
            };
        }

        // init default AccessTokenCookie credential
        if (!this.credentials['AccessTokenCookie']) {
            this.credentials['AccessTokenCookie'] = () => {
                if (this.apiKeys === null || this.apiKeys === undefined) {
                    return undefined;
                } else {
                    return this.apiKeys['AccessTokenCookie'] || this.apiKeys['access_token'];
                }
            };
        }
    }
